...
const Search = ({value}) => {
    ...
    const fetchDivisionData = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/district?divisionId=${selectedDivisionId}`);
            if (!response.ok) {
    ...
    const fetchUpogelaData = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/upogela?jelaId=${selectedJela}`);
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
            const data = await response.json();
    ...
    const fetchUnionData = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/unions?upogelaId=${selectedUpogela}`);
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
            const data = await response.json();
    ...
    return (
        <div>
            <div className=" my-4 shadow p-8 rounded-lg">
                ...
                <div className='flex justify-center items-center gap-5'>
                    ...
                    <div className="w-full mb-4">
                        <label htmlFor="jela" className="block text-sm mb-2">Jela</label>
                        <select
                            id="jela"
                            className="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
                            value={selectedJela}
                        ...
                        >
                            ...
                            {jelaData.filter(jela => jela.division_id === selectedDivisionId).map(jela => (
                    ...
                    <div className="w-full mb-4">
                        <label htmlFor="upogela" className="block text-sm mb-2">Upogela</label>
                        <select
                            id="upogela"
                            className="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
                    ...
                    <div className="w-full mb-4">
                        <label htmlFor="union" className="block text-sm mb-2">Union</label>
                        <select
                            id="union"
                            className="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
                            value={selectedUnion}
